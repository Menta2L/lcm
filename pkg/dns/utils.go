...
func updateDomainWithCName(r *dns.Msg, fqdn string) string {
	for _, rr := range r.Answer {
		if cn, ok := rr.(*dns.CNAME); ok {
			if cn.Hdr.Name == fqdn {
				log.Infof("Updating FQDN: %s with its CNAME: %s", fqdn, cn.Target)
				fqdn = cn.Target
				break
			}
		}
...
func checkAuthoritativeNss(fqdn, value string, nameservers []string) (bool, error) {
	for _, ns := range nameservers {
		...
		log.Infof("Looking up TXT records for %q", fqdn)
		var found bool
		for _, rr := range r.Answer {
...
func lookupNameservers(fqdn string, nameservers []string) ([]string, error) {
	...
	log.Infof("Searching fqdn %q using seed nameservers [%s]", fqdn, strings.Join(nameservers, ", "))
	...
	if len(authoritativeNss) > 0 {
		log.Infof("Returning authoritative nameservers [%s]", strings.Join(authoritativeNss, ", "))
...
func DNSQuery(fqdn string, rtype uint16, nameservers []string, recursive bool) (in *dns.Msg, err error) {
	...
	for i := 1; i <= len(nameservers)+1; i++ {
		...
		if err != nil && strings.HasPrefix(err.Error(), "read udp") && strings.HasSuffix(err.Error(), "i/o timeout") {
			log.Infof("UDP dns lookup failed, retrying with TCP: %v", err)
...
func FindZoneByFqdn(fqdn string, nameservers []string) (string, error) {
	...
	if zone, ok := fqdnToZone[fqdn]; ok {
		...
		log.Infof("Returning cached zone record %q for fqdn %q", zone, fqdn)
		return zone, nil
	}
	fqdnToZoneLock.RUnlock()
		...
		if in.Rcode == dns.RcodeSuccess {
			...
			for _, ans := range in.Answer {
				if soa, ok := ans.(*dns.SOA); ok {
					...
					log.Infof("Returning discovered zone record %q for fqdn %q", zone, fqdn)
					return zone, nil
				}
			}
		}
